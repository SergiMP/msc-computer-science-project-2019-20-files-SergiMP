let sa_queries = {
  updateCategory: `UPDATE staging_area.max_category SET MAX_category = (SELECT MAX(category_id) FROM staging_area.sa_d_account_category)`,

  updateClientManager: `UPDATE staging_area.max_client_manager SET max_manager = (SELECT MAX(client_manager_id)FROM staging_area.sa_d_client_manager)`,

  updateMaxCustomer: `UPDATE staging_area.max_customer SET max_customer = (SELECT MAX(customer_id) FROM staging_area.sa_d_customer)`,

  updateMaxDate: `UPDATE staging_area.max_date SET MAX_DATE = (SELECT MAX(calendar_date) FROM staging_area.sa_d_calendar)`,

  updateMaxLocation: `UPDATE staging_area.max_location SET max_location = (SELECT MAX(location_id) FROM staging_area.sa_d_location)`,

  updateMaxPromotion: `UPDATE staging_area.max_promotion SET max_promotion = (SELECT MAX(promotion_id) FROM staging_area.sa_d_promotion)`,

  updateMaxSubscription: `UPDATE staging_area.max_subscription_type SET max_subscription = (SELECT MAX(subscription_id) FROM staging_area.sa_d_subscription_type)`,
  ...
  insertCalendar: `staging_area.sa_create_incremental_calendar();`,

  insertFactComplaints: `SELECT staging_area.incremental_f_complaints()`,
